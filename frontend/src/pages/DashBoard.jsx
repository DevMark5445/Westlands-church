    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <div className="hidden sm:block text-left">
          <p className="text-sm font-semibold text-gray-800 leading-tight">{firstName} {lastName}</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
      {cards.map(card => {
        ...
        return (
          <div
            key={card.label}
            className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-lg hover:border-gray-200 transition-all duration-300 hover:-translate-y-0.5 cursor-default"
          >
        ...
        {data.map(c => (
          <li key={c.id} className="px-4 py-4">
            <div className="flex items-start justify-between mb-2">
              ...
              <button className="text-gray-400 hover:text-amber-600 transition-colors p-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                  <polyline points="7 10 12 15 17 10"/>
                  <line x1="12" y1="15" x2="12" y2="3"/>
            ...
            <li key={ev.id} className="px-6 py-4 flex items-start gap-4 hover:bg-gray-50/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CAT_STYLES[ev.category] || "bg-gray-100 text-gray-600"}`}>
                    {ev.category}
                  </span>
                </div>
          ...
          return (
            <li key={a.id} className="px-6 py-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {a.amount && (
                  <p className="text-xs font-semibold mt-0.5" style={{ color: "#c9a84c" }}>
                    {fmtCur(a.amount)}
                  </p>
                )}
              </div>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <h2 className="text-white text-2xl sm:text-3xl font-bold leading-tight mb-2 truncate">
              {firstName} {lastName} 👋
            </h2>
            <p className="text-white/60 text-sm mb-1 truncate">{user?.department || "Westlands P.A.G"} · {user?.role || "Member"}</p>
            <p className="text-white/40 text-xs truncate">Email: {user?.email || "—"}</p>
          </div>
          <div
            className="hidden sm:flex flex-col items-center justify-center flex-shrink-0 rounded-2xl px-5 py-4 border text-center"
            style={{ background: "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.12)", backdropFilter: "blur(8px)", minWidth: 120 }}
          >
            <p className="text-white/50 text-xs uppercase tracking-wider mb-1 whitespace-nowrap">Member Since</p>
            <p className="font-bold text-lg whitespace-nowrap" style={{ color: "#e8c876" }}>
              {user?.createdAt
